...
export default function MasterDataPage() {
  ...
  const handleSimulatedUpload = () => {
    ...
    setTimeout(() => {
      setUploading(false);
      setUploadResult('Successfully parsed and imported 42 new SKUs to the Products Master database.');
    }, 1200);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="glass-card px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center gap-4 bg-slate-50/50 dark:bg-slate-950/40">
          <div className="flex flex-col">
            <span className="text-[10px] font-extrabold uppercase text-slate-400">Bulk Data ingestion</span>
            <span className="text-[9px] text-slate-500 dark:text-slate-400 mt-0.5">Drag CSV catalogs to sync SKUs</span>
          </div>
          <button
            onClick={handleSimulatedUpload}
            disabled={uploading}
            className="px-3.5 py-1.5 bg-cyan-600 hover:bg-cyan-700 disabled:bg-cyan-700/50 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
          >
            <Upload className="w-3.5 h-3.5" />
            <span>{uploading ? 'Parsing CSV...' : 'Simulate CSV Upload'}</span>
          </button>
        </div>
      </div>
