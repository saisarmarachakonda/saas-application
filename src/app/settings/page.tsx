...
import { Settings, CreditCard, Check, Shield, Activity, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6 w-full max-w-none">
      ...
      {feedback && (
        <div className="p-3 bg-green-500/10 border border-green-500/20 text-xs text-green-500 rounded-lg flex items-center gap-2">
          <Check className="w-4 h-4 text-green-550 bg-green-500/20 rounded-full p-0.5" />
          <span>{feedback}</span>
        </div>
      )}

      <div className={initialView === 'general' ? 'w-full' : 'grid grid-cols-1 md:grid-cols-3 gap-6'}>
        <div className={initialView === 'general' ? 'w-full' : 'md:col-span-2 space-y-6'}>
                  ...
                  {(sett.value === 'true' || sett.value === 'false') ? (
                    <button
                      ...
                      className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${
                        sett.value === 'true' ? 'bg-indigo-650' : 'bg-slate-350 dark:bg-slate-800'
                      }`}
                    >
                ...
                <div className="pt-3 border-t border-slate-200 dark:border-slate-850 space-y-2">
                  ...
                  {subscription.planName !== 'Enterprise' && (
                    <button
                      ...
                      disabled={updating}
                      className="w-full py-2 bg-indigo-650 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1.5 glow-indigo cursor-pointer"
                    >
                  ...
                  {subscription.planName === 'Free' && (
                    <button
                      ...
                      disabled={updating}
                      className="w-full py-2 border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-xs font-semibold rounded-lg transition-colors cursor-pointer"
                    >
                  ...
                  {subscription.planName !== 'Free' && (
                    <button
                      ...
                      disabled={updating}
                      className="w-full py-2 border border-slate-200 dark:border-slate-850 text-slate-400 text-xs hover:text-red-500 rounded-lg transition-colors cursor-pointer"
                    >
                      Downgrade to Free
                    </button>
                  )}
