...
import { Boxes, Scan, CheckCircle2 } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const handleSimulatedScan = () => {
    ...
    setTimeout(() => {
      setScanning(false);
      setScanResult('RFID Node #30291 matched: STM32 Microcontroller - SKU: MCU-STM32-H7. Verified 250 units in Chicago Central Depot Bin B-12.');
    }, 1000);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="glass-card px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center gap-4 bg-slate-50/50 dark:bg-slate-950/40">
          <div className="flex flex-col">
            <span className="text-[10px] font-extrabold uppercase text-slate-400">Handheld Scan Utility</span>
            <span className="text-[9px] text-slate-500 dark:text-slate-400 mt-0.5">Mock scan of active bin tags</span>
          </div>
          <button
            onClick={handleSimulatedScan}
            disabled={scanning}
            className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-700/50 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
          >
            <Scan className="w-3.5 h-3.5 animate-pulse" />
            <span>{scanning ? 'Scanning Tag...' : 'Simulate RFID Scan'}</span>
          </button>
        </div>
      </div>

      {scanResult && (
        <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-450 text-xs rounded-xl flex items-center gap-2">
