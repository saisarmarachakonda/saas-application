...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      console.warn(`[AUTH RESILIENT] Database check bypassed or timed out:`, e);
    }

    // Mock database fallback (with dynamic on-the-fly user generation)
    if (!user) {
      const mockUsers = initialMockDb.user;
      let foundMock = mockUsers.find(u => u.email === email);
      
      if (!foundMock) {
        // Generate mock credentials on the fly for friction-free simulation
    ...
    // Frictionless bypass: Accept any password for mock accounts in prototype mode
    ...
    try {
      await db.activityLog.create({
        data: {
          ...
          action: 'LOGIN',
          details: `Successful user login to ${app.toUpperCase()} portal`,
    ...
    response.cookies.set({
      name: `${app.toLowerCase()}_auth_token`,
      value: token,
      httpOnly: true,
      secure: isSecure,
      sameSite: 'lax',
